...
{
	public class iyzicoExpressController : PaymentControllerBase
	{
		...
		public iyzicoExpressController(
            ...
            IStoreService storeService)
		{
			...
			_helper = new PluginHelper(ctx, "EARGE.iyzico", "Plugins.Payments.iyzicoExpress");
		...
		{
            var model = new iyzicoExpressConfigurationModel();
            int storeScope = this.GetActiveStoreScopeConfiguration(_storeService, _services.WorkContext);
            var settings = _services.Settings.LoadSetting<iyzicoExpressPaymentSettings>(storeScope);
		...
		public ActionResult Configure(iyzicoExpressConfigurationModel model, FormCollection form)
		{
		...
		public ActionResult PaymentInfo()
		{
			...
			var model = new iyzicoExpressPaymentInfoModel();
			model.CurrentPageIsBasket = iyzicoHelper.CurrentPageIsBasket(this.ControllerContext.ParentActionViewContext.RequestContext.RouteData);
			...
			if (model.CurrentPageIsBasket)
			{
				...
				model.SubmitButtonImageUrl = iyzicoHelper.CheckIfButtonExists(buttonUrl);
			}
		...
		public ActionResult IPNHandler()
		{
			...
			var provider = _paymentService.LoadPaymentMethodBySystemName("Payments.iyzicoExpress", true);
			var processor = provider != null ? provider.Value as iyzicoExpress : null;
			if (processor == null)
				throw new SmartException(T("iyzico Express module cannot be loaded"));
			...
			if (processor.VerifyIPN(strRequest, out values))
			{
				...
				var newPaymentStatus = iyzicoHelper.GetPaymentStatus(payment_status, pending_reason);
					...
					case "recurring_payment":
						#region Recurring payment
						{
							...
							if (initialOrder != null)
							{
								...
								_logger.Information("iyzico IPN. Recurring info", new SmartException(sb.ToString()));
							}
							else
							{
								_logger.Error("iyzico IPN. Order is not found", new SmartException(sb.ToString()));
							}
						}
						#endregion
						break;
			...
			{
				_logger.Error("iyzico IPN failed.", new SmartException(strRequest));
			}
		...
		public ActionResult SubmitButton()
		{
			try
			{
				...
				var settings = _services.Settings.LoadSetting<iyzicoExpressPaymentSettings>(_services.StoreContext.CurrentStore.Id);
                ...
                if (String.IsNullOrEmpty(settings.ApiAccountName))
					throw new ApplicationException("iyzico API Account Name is not set");
                if (String.IsNullOrEmpty(settings.ApiAccountPassword))
					throw new ApplicationException("iyzico API Password is not set");
                if (String.IsNullOrEmpty(settings.Signature))
					throw new ApplicationException("iyzico API Signature is not set");

				var provider = _paymentService.LoadPaymentMethodBySystemName("Payments.iyzicoExpress", true);
				var processor = provider != null ? provider.Value as iyzicoExpress : null;
				if (processor == null)
					throw new SmartException("iyzico Express Checkout module cannot be loaded");

				var processPaymentRequest = new iyzicoProcessPaymentRequest();
				...
				processPaymentRequest.PaymentMethodSystemName = "Payments.iyzicoExpress";
				...
				if (resp.Ack == AckCodeType.Success)
				{
					...
					processPaymentRequest.IsShippingMethodSet = iyzicoHelper.CurrentPageIsBasket(this.RouteData);
					this.Session["OrderPaymentInfo"] = processPaymentRequest;

					_genericAttributeService.SaveAttribute<string>(customer, SystemCustomerAttributeNames.SelectedPaymentMethod, "Payments.iyzicoExpress",
						_services.StoreContext.CurrentStore.Id);

					var result = new RedirectResult(String.Format(
                        iyzicoHelper.GetPaypalUrl(settings) + 
		...
		public ActionResult GetDetails(string token)
		{
			...
			var processor = provider != null ? provider.Value as iyzicoExpress : null;
			if (processor == null)
				throw new SmartException("iyzico Express module cannot be loaded");
		...
		public override IList<string> ValidatePaymentForm(FormCollection form)
		{
			...
			//validate
			var validator = new iyzicoExpressPaymentInfoValidator(_localizationService);
			var model = new iyzicoExpressPaymentInfoModel()
			{
