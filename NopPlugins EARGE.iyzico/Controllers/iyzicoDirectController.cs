...
{
	public class iyzicoDirectController : PaymentControllerBase
	{
            ...
            IStoreService storeService)
		{
			...
			_helper = new PluginHelper(ctx, "EARGE.iyzico", "Plugins.Payments.iyzicoDirect");
		}
		...
		{
            var model = new iyzicoDirectConfigurationModel();
            int storeScope = this.GetActiveStoreScopeConfiguration(_storeService, _services.WorkContext);
            var settings = _services.Settings.LoadSetting<iyzicoDirectPaymentSettings>(storeScope);
		...
		public ActionResult Configure(iyzicoDirectConfigurationModel model, FormCollection form)
		{
		...
		{
			Debug.WriteLine("iyzico Direct IPN: {0}".FormatWith(Request.ContentLength));

			byte[] param = Request.BinaryRead(Request.ContentLength);
			string strRequest = Encoding.ASCII.GetString(param);
			Dictionary<string, string> values;

			var provider = _paymentService.LoadPaymentMethodBySystemName("Payments.iyzicoDirect", true);
			var processor = provider != null ? provider.Value as iyzicoDirectProvider : null;
			if (processor == null)
